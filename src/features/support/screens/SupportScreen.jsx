...
function InfoCard({ data, colors }) {
    return (<View style={[styles.infoCard, { backgroundColor: colors.surface, borderColor: colors.border }]}>
...
function Dropdown({ label, value, options, onSelect, colors, inputBg, }) {
      ...
      <Modal visible={open} transparent animationType="fade">
        <TouchableOpacity style={styles.modalOverlay} activeOpacity={1} onPress={() => setOpen(false)}>
          <View style={[styles.modalContent, { backgroundColor: colors.surface, borderColor: colors.border }]}>
            <FlatList data={options} keyExtractor={(item) => item} renderItem={({ item }) => (<TouchableOpacity style={styles.modalOption} onPress={() => {
                onSelect(item);
                setOpen(false);
            }}>
...
export default function ContactSupport({ onBackPress }) {
    ...
    const handleSend = async () => {
        ...
        try {
            await axios.post(`${API_BASE_URL}/api/client/email/send-notification`, { name, email, category, priority, subject, message }, {
      ...
      <View style={[styles.formCard, { backgroundColor: colors.surface, borderColor: colors.border }]}>
        ...
        <View style={styles.fieldGroup}>
          ...
          <View style={[styles.inputWrapper, { backgroundColor: inputBg, borderColor: colors.inputBorder }]}>
            ...
            <TextInput style={[styles.input, { color: colors.text }]} placeholder="John Doe" placeholderTextColor={colors.inputPlaceholder} value={name} onChangeText={setName}/>
          </View>
        </View>
        ...
        <View style={styles.fieldGroup}>
          ...
          <View style={[styles.inputWrapper, { backgroundColor: inputBg, borderColor: colors.inputBorder }]}>
            ...
            <TextInput style={[styles.input, { color: colors.text }]} placeholder="john@example.com" placeholderTextColor={colors.inputPlaceholder} value={email} onChangeText={setEmail} keyboardType="email-address" autoCapitalize="none"/>
          </View>
        </View>

        <Dropdown label="Category" value={category} options={categoryOptions} onSelect={setCategory} colors={colors} inputBg={inputBg}/>

        <Dropdown label="Priority" value={priority} options={priorityOptions} onSelect={setPriority} colors={colors} inputBg={inputBg}/>
        ...
        <View style={styles.fieldGroup}>
          ...
          <View style={[styles.inputWrapperPlain, { backgroundColor: inputBg, borderColor: colors.inputBorder }]}>
            <TextInput style={[styles.input, { color: colors.text }]} placeholder="Brief summary of your issue" placeholderTextColor={colors.inputPlaceholder} value={subject} onChangeText={setSubject}/>
          </View>
        </View>
        ...
        <View style={styles.fieldGroup}>
          ...
          <View style={[styles.textareaWrapper, { backgroundColor: inputBg, borderColor: colors.inputBorder }]}>
            <TextInput style={[styles.textarea, { color: colors.text }]} placeholder="Please describe your issue in detail..." placeholderTextColor={colors.inputPlaceholder} value={message} onChangeText={setMessage} multiline numberOfLines={5} textAlignVertical="top"/>
          </View>
        ...
        <View style={styles.fieldGroup}>
          ...
          {selectedFiles.length > 0 && (<View style={{ marginTop: 8, gap: 4 }}>
              {selectedFiles.map((file, index) => (<Text key={index} numberOfLines={1} style={{ fontSize: font.sm, color: colors.text }}>{'\u2022'} {file.name}</Text>))}
            </View>)}
        </View>
