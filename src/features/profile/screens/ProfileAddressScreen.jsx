...
function ProfileAddressScreen() {
    ...
    const [street, setStreet] = useState(user?.address?.addressLine1 ?? user?.address?.street ?? user?.addressLine1 ?? user?.street ?? '');
    const initialCityState = [user?.address?.city, user?.address?.state]
        .filter(Boolean)
        .join(', ');
    const [cityState, setCityState] = useState(initialCityState || user?.state || '');
    const [postalCode, setPostalCode] = useState(user?.address?.postalCode ?? user?.postalCode ?? '');
