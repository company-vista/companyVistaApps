...
function StepProgress({ total, current }) {
    ...
    return (<View style={styles.progressRow}>
      {Array.from({ length: total }).map((_, idx) => (<View key={idx} style={[
...
function RadioOption({ option, selected, onSelect, colors, }) {
    return (<TouchableOpacity style={[
            styles.radioCard,
            { backgroundColor: colors.surface, borderColor: colors.border },
            selected && { borderColor: '#e6a82a', backgroundColor: 'rgba(230,168,42,0.06)' },
        ]} onPress={() => onSelect(option.id)} activeOpacity={0.8}>
...
export default function AddCompanyScreen({ onBackPress, onSubmit: onFormSubmit, companyId: editingCompanyId }) {
    ...
    const [selectedJurisdiction, setSelectedJurisdiction] = useState(isEditing ? reg.jurisdiction : null);
    useEffect(() => {
        if (!editingCompanyId || !token)
            return;
        let cancelled = false;
        (async () => {
            setLoading(true);
            ...
            if (result.isSuccess && result.company) {
                ...
                if (c.email)
                    setEmail(c.email);
                if (c.phone || c.phoneNumber || c.mobile)
                    setPhone(c.phone || c.phoneNumber || c.mobile);
                if (c.jurisdiction || c.countryOfIncorporation)
                    setSelectedJurisdiction(c.jurisdiction ?? null);
                if (c.applicantType)
                    setApplicantType(c.applicantType);
            }
            setLoading(false);
        })();
    ...
    if (step === 2) {
        return (<JurisdictionSelectionScreen onBackPress={() => setStep(1)} onContinue={(countryCode, countryName) => {
    ...
    if (step === 3) {
        return (<EntityDetailScreen selectedJurisdiction={selectedJurisdiction} onBackPress={() => setStep(2)} onContinue={() => setStep(4)}/>);
    }
    if (step === 4) {
    ...
    if (step === 9) {
        return (<ReviewSubmitScreen onBackPress={() => setStep(8)} onSubmit={(companyId) => onFormSubmit?.(companyId)} onEditApplicant={() => setStep(1)} onEditJurisdiction={() => setStep(2)} onEditCompanyName={() => setStep(3)} onEditOwnership={() => setStep(4)} onEditAddress={() => setStep(5)} onEditDirectors={() => setStep(6)} onEditBusinessActivity={() => setStep(7)} companyId={editingCompanyId}/>);
    }
    ...
    return (<View style={styles.screen}>
      ...
      <ScrollView contentContainerStyle={[styles.content, { paddingBottom: safeAreaInsets.bottom + 24 }]}>
        ...
        <View style={styles.radioGroup}>
          {applicantOptions.map((option) => (<RadioOption key={option.id} option={option} selected={applicantType === option.id} onSelect={setApplicantType} colors={colors}/>))}
        </View>
        ...
        <View style={styles.row}>
          <View style={styles.fieldHalf}>
            ...
            <View style={[styles.inputWrapper, { backgroundColor: inputBg, borderColor: colors.inputBorder }]}>
              <TextInput style={[styles.input, { color: colors.text }]} value={firstName} onChangeText={setFirstName} placeholder="First name" placeholderTextColor={colors.inputPlaceholder}/>
            </View>
          </View>
          <View style={styles.fieldHalf}>
            <Text style={[styles.fieldLabel, { color: colors.muted }]}>LAST NAME</Text>
            <View style={[styles.inputWrapper, { backgroundColor: inputBg, borderColor: colors.inputBorder }]}>
              <TextInput style={[styles.input, { color: colors.text }]} value={lastName} onChangeText={setLastName} placeholder="Last name" placeholderTextColor={colors.inputPlaceholder}/>
            </View>
          </View>
        </View>
