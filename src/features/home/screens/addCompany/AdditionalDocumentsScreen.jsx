...
export default function AdditionalDocumentsScreen({ onBackPress, onContinue }) {
    const safeAreaInsets = useSafeAreaInsets();
    const colors = useThemeColors();
    const dispatch = useAppDispatch();
    const [holdingFiles, setHoldingFiles] = useState([]);
    const [otherFiles, setOtherFiles] = useState([]);
    const [attemptedSubmit, setAttemptedSubmit] = useState(false);
    const handleUploadPress = async (boxType) => {
        try {
    ...
    return (<View style={styles.screen}>
      ...
      <View style={styles.body}>
        <ScrollView style={styles.scrollArea} contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
          ...
          <TouchableOpacity style={[styles.uploadBox, { borderColor: attemptedSubmit && holdingFiles.length === 0 ? '#ef4444' : '#e6a82a', backgroundColor: colors.mode === 'dark' ? 'rgba(13,22,39,1)' : colors.surfaceAlt }]} activeOpacity={0.7} onPress={() => handleUploadPress('holding')}>
          ...
          {holdingFiles.length > 0 && (<View style={styles.selectedFilesContainer}>
              {holdingFiles.map((f, i) => (<Text key={i} style={[styles.selectedFileText, { color: colors.text }]}>
                  {'\u2713'} {f.name}
                </Text>))}
            </View>)}
          ...
          <TouchableOpacity style={[styles.uploadBox, { borderColor: attemptedSubmit && otherFiles.length === 0 ? '#ef4444' : '#e6a82a', backgroundColor: colors.mode === 'dark' ? 'rgba(13,22,39,1)' : colors.surfaceAlt }]} activeOpacity={0.7} onPress={() => handleUploadPress('other')}>
            <Text style={styles.uploadIcon}>{'\u21E7'}</Text>
            <Text style={[styles.uploadMainText, { color: colors.text }]}>Click or drag other documents</Text>
            <Text style={[styles.uploadSubText, { color: colors.subtle }]}>JPG, PNG, PDF — multiple files allowed</Text>
          </TouchableOpacity>

          {otherFiles.length > 0 && (<View style={styles.selectedFilesContainer}>
              {otherFiles.map((f, i) => (<Text key={i} style={[styles.selectedFileText, { color: colors.text }]}>
                  {'\u2713'} {f.name}
                </Text>))}
            </View>)}
