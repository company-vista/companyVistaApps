...
function RegionTab({ label, active, onPress, colors, }) {
...
function CountryRow({ country, selected, onSelect, colors, }) {
    return (<TouchableOpacity style={[
            styles.countryCard,
            { backgroundColor: colors.surface, borderColor: colors.border },
            selected && { borderColor: '#e6a82a', backgroundColor: colors.surfaceAlt },
        ]} onPress={() => onSelect(country.code)} activeOpacity={0.8}>
      ...
      {selected ? (<View style={styles.checkCircle}>
          <Text style={styles.checkMark}>✓</Text>
        </View>) : null}
    </TouchableOpacity>);
}
export default function JurisdictionSelectionScreen({ onBackPress, onContinue }) {
    const safeAreaInsets = useSafeAreaInsets();
    const colors = useThemeColors();
    const [search, setSearch] = useState('');
    const [activeRegion, setActiveRegion] = useState('ALL');
    const [selectedCountry, setSelectedCountry] = useState(countries.find((c) => c.code === 'CR') ?? null);
      ...
      <View style={styles.body}>
        <ScrollView style={styles.scrollArea} contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
          ...
          <View style={[styles.searchWrapper, { backgroundColor: inputBg, borderColor: colors.inputBorder }]}>
            ...
            <TextInput style={[styles.searchInput, { color: colors.text }]} value={search} onChangeText={setSearch} placeholder="Search countries..." placeholderTextColor={colors.inputPlaceholder}/>
          </View>
          ...
          <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.regionScroll} contentContainerStyle={styles.regionScrollContent}>
            {regionTabs.map((tab) => (<RegionTab key={tab.id} label={tab.label} active={activeRegion === tab.id} onPress={() => setActiveRegion(tab.id)} colors={colors}/>))}
          </ScrollView>

          {filteredCountries.map((item) => (<CountryRow key={item.code} country={item} selected={selectedCountry?.code === item.code} onSelect={(code) => setSelectedCountry(countries.find((c) => c.code === code) ?? null)} colors={colors}/>))}
        </ScrollView>

        {selectedCountry ? (<View style={[styles.banner, { backgroundColor: 'rgba(230,168,42,0.08)', borderColor: '#6b5320' }]}>
