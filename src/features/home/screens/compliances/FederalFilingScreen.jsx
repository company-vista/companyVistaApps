...
export default function FederalTaxFiling() {
    ...
    const [selectedFinancialStatements, setSelectedFinancialStatements] = useState([]);
    const [submitting, setSubmitting] = useState(false);
    const [submissionStatus, setSubmissionStatus] = useState(null);
    const [selectedCompanyId, setSelectedCompanyId] = useState(selectedAction?.companyId ?? null);
    const companiesLoadedKeyRef = useRef(null);
    ...
    useEffect(() => {
        ...
        const loadCompanies = async () => {
            ...
            try {
                ...
                const options = buildCompanyOptions(loadedCompanies);
    ...
    const removeSelectedFile = (type, indexToRemove) => {
        if (type === 'bank') {
    ...
    return (<View style={styles.container}>
      <ScrollView contentContainerStyle={styles.scrollContainer} showsVerticalScrollIndicator={false}>
        ...
        <View style={[styles.card, { backgroundColor: colors.surface, zIndex: 20 }]}>
          ...
          <Pressable style={[styles.dropdownSelector, { borderColor: colors.border, backgroundColor: colors.background }]} onPress={() => setIsCompanyDropdownOpen(prev => !prev)}>
          ...
          {isCompanyDropdownOpen ? (<View style={[styles.dropdownList, { borderColor: colors.border, backgroundColor: colors.surface }]}>
              {companyOptions.length > 0 ? (companyOptions.map(option => (<Pressable key={option.id} style={[styles.dropdownItem, { borderBottomColor: colors.border }]} onPress={() => {
                  ...
                  </Pressable>))) : (<View style={[styles.dropdownItem, { borderBottomColor: colors.border }]}>
        ...
        <View style={[styles.card, { backgroundColor: colors.surface, zIndex: 10 }]}>
          ...
          <Pressable style={[styles.dropdownSelector, { borderColor: colors.border, backgroundColor: colors.background }]} onPress={() => setIsTaxYearDropdownOpen(prev => !prev)}>
          ...
          {isTaxYearDropdownOpen ? (<View style={[styles.dropdownList, { borderColor: colors.border, backgroundColor: colors.surface }]}>
              {taxYearOptions.map(option => (<Pressable key={option} style={[styles.dropdownItem, { borderBottomColor: colors.border }]} onPress={() => {
                    setTaxYear(option);
        ...
        <View style={[styles.card, styles.rowBetween, { backgroundColor: colors.surface }]}>
          ...
          <Switch value={bookkeeping} onValueChange={(value) => setBookkeeping(value)} trackColor={{ false: colors.border, true: colors.primary }} thumbColor={bookkeeping ? colors.accent : colors.surface}/>
        </View>
        ...
        <View style={[styles.card, { backgroundColor: colors.surface }]}>
          ...
          <Pressable style={({ pressed }) => [styles.uploadArea, { borderColor: colors.border, backgroundColor: colors.background }, pressed && { opacity: 0.7 }]} onPress={() => handlePickDocument('bank')}>
            ...
            {selectedBankStatements.length > 0 ? (<View style={styles.fileList}>
                {selectedBankStatements.map((file, index) => (<View key={`${file.uri}-${index}`} style={[styles.fileItem, { backgroundColor: colors.background }]}>
        ...
        {bookkeeping ? (<View style={[styles.card, { backgroundColor: colors.surface }]}>
            ...
            <Pressable style={({ pressed }) => [styles.uploadArea, { borderColor: colors.border, backgroundColor: colors.background }, pressed && { opacity: 0.7 }]} onPress={() => handlePickDocument('financial')}>
              ...
              <Text style={[styles.uploadSubText, { color: colors.muted }]}>PDF, JPG, PNG, DOC (Max 10MB each)</Text>
              {selectedFinancialStatements.length > 0 ? (<View style={styles.fileList}>
                  {selectedFinancialStatements.map((file, index) => (<View key={`${file.uri}-${index}`} style={[styles.fileItem, { backgroundColor: colors.background }]}>
                      <FontAwesome name="file" size={12} color={colors.accent}/>
                      <Text numberOfLines={1} style={[styles.fileItemText, { color: colors.text }]}>{file.name}</Text>
        ...
        <View style={[styles.card, { backgroundColor: colors.surface }]}>
          ...
          <TextInput style={[styles.textArea, { borderColor: colors.border, backgroundColor: colors.background, color: colors.text }]} multiline={true} numberOfLines={4} placeholder="Any additional information you'd like to share with our tax team..." placeholderTextColor={colors.inputPlaceholder} value={additionalNotes} onChangeText={(text) => setAdditionalNotes(text)}/>
        </View>

    {/* Submit Button */}
        <Pressable style={[styles.submitButton, { backgroundColor: colors.buttonBackground, shadowColor: colors.accent }, submitting && styles.submitButtonDisabled]} onPress={handleSubmit} disabled={submitting}>
