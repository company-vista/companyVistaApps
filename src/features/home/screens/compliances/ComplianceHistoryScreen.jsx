...
const ComplianceHistoryScreen = () => {
    ...
    const matchesSelectedAction = (record) => {
    ...
    const getHistoryDueDate = (record) => {
        const segment = getHistorySegment(record);
        return (segment.dueDate ??
            segment.due_date ??
            segment.duedate ??
            segment.due ??
            segment.deadline ??
    ...
    const getHistoryLastDate = (record) => {
        const segment = getHistorySegment(record);
        return (segment.lastDate ??
            segment.last_date ??
            segment.lastFiled ??
            segment.last_filed ??
            segment.completedAt ??
    ...
    const pendingHistory = filteredHistory.some(record => getHistoryStatus(record).toLowerCase().includes('pending'));
    const showRenewButton = String(selectedAction.status ?? '').toLowerCase().includes('pending') || pendingHistory;
    const formatRecordValue = (value, isDate = false) => {
