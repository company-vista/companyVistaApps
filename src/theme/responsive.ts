...
export function useResponsive() {
  ...
  const rs = (size: number) => PixelRatio.roundToNearestPixel(size * scaleW);
  const rvs = (size: number) => PixelRatio.roundToNearestPixel(size * scaleH);
  const rms = (size: number, factor = 0.5) => size + (rs(size) - size) * factor;
